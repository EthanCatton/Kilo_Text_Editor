...
void editorUpdateRow(erow *row){
	...
	for(j=0;j<row->size;j++){
		if(row->chars[j]=='\t'){
			row->render[idx++]=' ';
			while(idx % KILO_TAB_STOP!=0)row->render[idx++]=' ';}else{
...
void editorDrawMessageBar(struct abuf *ab){
	...
	//These lines appear to hide the message bar regardless - I cannot see any direct errors
	//so, as it does no harm, these shall remain removed.
	...
	while(1){
		...
		if(c==DEL_KEY||c==CTRL_KEY('h')||c==BACKSPACE){
			...
			if(buflen!=0){
				editorSetStatusMessage("");
				if(callback) callback(buf,c);
				return buf;}}else if(!iscntrl(c) && c<128){
					if(buflen==bufsize-1){
						bufsize*=2;
